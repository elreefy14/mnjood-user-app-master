...
import 'package:mnjood_delivery/feature/chat/domain/models/message_model.dart';
...
import 'package:mnjood_delivery/helper/image_size_checker.dart';
...
class ChatController extends GetxController implements GetxService {
  ...
  Future<void> getAdminMessages() async {
    ...
    if (response != null && response.statusCode == 200 && response.body['messages'] != {}) {
      MessageModel adminMessageModel = MessageModel.fromJson(response.body);
      if (adminMessageModel.conversation != null) {
        _adminConversationModel = adminMessageModel.conversation;
        if (_adminConversationModel!.receiverType == 'delivery_man') {
          User? receiver = _adminConversationModel!.receiver;
          _adminConversationModel!.receiver = _adminConversationModel!.sender;
          _adminConversationModel!.sender = receiver;
        }
        _adminConversationModel!.receiver ??= User(
          id: 0,
          fName: Get.find<SplashController>().configModel?.businessName ?? 'Admin',
        );
      }
    }
    update();
  ...
  Future<void> getMessages(int offset, NotificationBodyModel notificationBody, User? user, int? conversationID, {bool firstLoad = false}) async {
    ...
    if(notificationBody.adminId != null || notificationBody.type == UserType.admin.name) {
      response = await chatServiceInterface.getMessages(offset, notificationBody.adminId ?? 0, UserType.admin, conversationID);
    }else if(notificationBody.customerId != null || notificationBody.type == UserType.user.name) {
    ...
    if (response != null && response.body['messages'] != {} && response.statusCode == 200) {
      if (offset == 1) {
        ...
        if(notificationBody.adminId != null && _messageModel!.conversation != null) {
...
/*  void pickImage(bool isRemove) async {
  ...
  Future<Response?> sendMessage({required String message, required NotificationBodyModel? notificationBody, required int? conversationId}) async {
    ...
    if(notificationBody != null && (notificationBody.adminId != null || notificationBody.type == UserType.admin.name)) {
      response = await chatServiceInterface.sendMessage(message, myImages, conversationId, notificationBody.adminId ?? 0, UserType.admin);
    }else if(notificationBody != null && (notificationBody.customerId != null || notificationBody.type == UserType.user.name)) {
