...
class DateConverter {
  ...
  /// Normalize ISO 8601 date string to standard format
  /// Handles formats like: 2026-01-07T13:48:37.000000Z, 2026-01-07T13:48:37Z, 2026-01-07 13:48:37
  static String _normalizeDateTime(String dateTime) {
    if (dateTime.isEmpty) return dateTime;
    // Remove Z suffix and replace T with space
    String normalized = dateTime.replaceAll('Z', '').replaceAll('T', ' ');
  ...
  static String dateTimeStringToDateTime(String dateTime) {
    return DateFormat('dd MMM yyyy  ${_timeFormatter()}').format(DateFormat('yyyy-MM-dd HH:mm:ss').parse(_normalizeDateTime(dateTime)));
  }

  static String dateTimeStringToDateTimeNewFormat(String dateTime) {
    return DateFormat('dd/MM/yyyy, ${_timeFormatter()}').format(DateFormat('yyyy-MM-dd HH:mm:ss').parse(_normalizeDateTime(dateTime)));
  }
  ...
  static String dateTimeStringToDateOnly(String dateTime) {
    return DateFormat('dd MMM yyyy').format(DateFormat('yyyy-MM-dd HH:mm:ss').parse(_normalizeDateTime(dateTime)));
  }
  ...
  static DateTime dateTimeStringToDate(String dateTime) {
    return DateFormat('yyyy-MM-dd HH:mm:ss').parse(_normalizeDateTime(dateTime));
  }
  ...
  static int differenceInMinute(String? deliveryTime, String? orderTime, int? processingTime, String? scheduleAt) {
    ...
    String dateTimeStr = scheduleAt ?? orderTime ?? '';
    if (dateTimeStr.isEmpty) {
      return 0;
    }
    DateTime deliveryTime0 = dateTimeStringToDate(dateTimeStr).add(Duration(minutes: minTime));
  ...
  static String dateTimeToDayMonthAndTime(String dateTime) {
    return DateFormat('dd MMM yyyy, ${_timeFormatter()}').format(dateTimeStringToDate(dateTime).toLocal());
  }
