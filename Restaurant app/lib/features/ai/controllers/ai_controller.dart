...
class AiController extends GetxController implements GetxService {
  ...
  Future<void> generateTitleAndDes({required String title, required String langCode}) async {
    ...
    TitleDesModel? titleDesModel = await aiServiceInterface.generateTitleAndDes(title: title, langCode: langCode, restaurantId: Get.find<ProfileController>().profileModel!.id.toString(), generateFrom: _requestType);
  ...
  Future<void> generateOtherData({required String title, required String description}) async {
    ...
    OtherDataModel? otherDataModel = await aiServiceInterface.generateOtherData(title: title, description: description, restaurantId: Get.find<ProfileController>().profileModel!.id.toString(), generateFrom: _requestType);
  ...
  Future<void> generateVariationData({required String title, required String description}) async {
    ...
    VariationDataModel? variationDataModel = await aiServiceInterface.generateVariationData(title: title, description: description, restaurantId: Get.find<ProfileController>().profileModel!.id.toString(), generateFrom: _requestType);
  ...
  Future<void> generateTitleSuggestions() async {
    ...
    TitleSuggestionModel? titleSuggestionModel = await aiServiceInterface.generateTitleSuggestions(keywords: keyWord, restaurantId: Get.find<ProfileController>().profileModel!.id.toString());
