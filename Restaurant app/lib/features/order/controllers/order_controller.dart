...
import 'package:mnjood_vendor/features/order/domain/models/running_order_model.dart';
import 'package:mnjood_vendor/features/subscription/domain/models/subscription_model.dart';
...
class OrderController extends GetxController implements GetxService {
  ...
  // Map of orderId -> Set of scanned item IDs (using foodId to identify unique items)
  final Map<int, Set<int>> _scannedItems = {};

  /// Check if a specific item has been scanned for an order
  bool isItemScanned(int orderId, int foodId) {
    return _scannedItems[orderId]?.contains(foodId) ?? false;
  }

  /// Mark an item as scanned for an order
  void markItemScanned(int orderId, int foodId) {
    _scannedItems[orderId] ??= {};
    _scannedItems[orderId]!.add(foodId);
    update();
  }

  /// Check if all items in the order have been scanned
  bool allItemsScanned(int orderId, List<OrderDetailsModel>? items) {
    if (items == null || items.isEmpty) return true;
    final scanned = _scannedItems[orderId] ?? {};
    // Check that each unique item (by foodId) has been scanned
    return items.every((item) =>
      item.foodId != null && scanned.contains(item.foodId)
    );
  }

  /// Get count of scanned items for an order
  int getScannedCount(int orderId) {
    return _scannedItems[orderId]?.length ?? 0;
  }

  /// Clear scanned items for an order (call when order is completed or reset)
  void clearScannedItems(int orderId) {
    _scannedItems.remove(orderId);
    update();
  }

  /// Find order item by barcode
  OrderDetailsModel? findItemByBarcode(String barcode, List<OrderDetailsModel>? items) {
    if (items == null) return null;
    for (final item in items) {
      if (item.foodDetails?.barcode == barcode) {
        return item;
      }
    }
    return null;
  ...
  Future<void> getOrderDetails(int orderID) async {
    ...
    if(response.statusCode == 200) {
      ...
      if(order != null && order['subscription'] != null){
        try {
          _subscriptionModel = SubscriptionModel.fromJson(order['subscription']);
        } catch (e) {
  ...
  void toggleCampaignOnly() {
    ...
    for (var order in _runningOrderList!) {
      bool selfDelivery = Get.find<ProfileController>().profileModel?.restaurants?.isNotEmpty == true && Get.find<ProfileController>().profileModel!.restaurants![0].selfDeliverySystem == 1;
      // Supermarkets and pharmacies should always be able to see/confirm pending orders
      bool isSupermarketOrPharmacy = BusinessTypeHelper.isSupermarket() || BusinessTypeHelper.isPharmacy();
      if(order.orderStatus == 'pending' && (Get.find<SplashController>().configModel?.orderConfirmationModel != 'deliveryman'
          || order.orderType == 'take_away' || selfDelivery || isSupermarketOrPharmacy)
