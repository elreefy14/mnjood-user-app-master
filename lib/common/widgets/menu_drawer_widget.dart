...
class MenuDrawerWidgetState extends State<MenuDrawerWidget> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Initialize controller FIRST before any code that might throw
    _staggeredController = AnimationController(
      vsync: this,
      duration: _animationDuration,
    );

    // Now safe to do config checks with null-safe access
    final config = Get.find<SplashController>().configModel;

    if(config?.refundPolicyStatus ?? false) {
      _menuList.add(Menu(icon: Images.refund, title: 'refund_policy'.tr, onTap: () {
        Get.offNamed(RouteHelper.getHtmlRoute('refund-policy'));
      }));
    }
    if(config?.cancellationPolicyStatus ?? false) {
    ...
    if(config?.shippingPolicyStatus ?? false) {
        ...
        children: [
          Container(
            ...
            child: Row( mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                IconButton( padding: const EdgeInsets.all(0), onPressed: () => Get.back(), icon: const Icon(HeroiconsOutline.xMark))
              ],
            ),
          ),
