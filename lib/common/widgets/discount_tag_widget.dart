...
class DiscountTagWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return !isProductBottomSheet ? ((discount ?? 0) > 0 || (freeDelivery ?? false)) ? Positioned(
      ...
      child: CustomPaint(
        ...
        child: Container(
          ...
          child: Align(
            child: Text(
              (discount ?? 0) > 0 ? '${(isRightSide || discountType == 'percent') ? '' : currencySymbol}$discount${discountType == 'percent' ? '%'
    ...
    ) : const SizedBox() : ((discount ?? 0) > 0 || (freeDelivery ?? false)) ? Positioned(
