...
class VegFilterWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    // Veg/Non-veg filter widget disabled - always return empty widget
    ...
    return (Get.find<SplashController>().configModel!.toggleVegNonVeg ?? false) ? Padding(
      ...
      child: PopupMenuButton<dynamic>(
        ...
        child: Container(
          ...
          child: Icon(HeroiconsOutline.adjustmentsVertical, size: fromAppBar ? 18 : 24, color: iconColor ?? Theme.of(context).textTheme.bodyMedium!.color),
        ),
      ),
    ) : const SizedBox();
    */
    ...
    for(int i=0; i < productTypeList.length; i++){
      entryList.add(PopupMenuItem<int>(value: i, child: Row(children: [
        productTypeList[i] == type
            ? Icon(HeroiconsSolid.checkCircle, color: Theme.of(context).primaryColor)
            : Icon(HeroiconsOutline.minusCircle, color: Theme.of(context).disabledColor),
