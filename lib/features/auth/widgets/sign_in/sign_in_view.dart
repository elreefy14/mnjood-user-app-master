...
import 'package:mnjood/features/auth/widgets/sign_in/manual_login_widget.dart';
import 'package:mnjood/features/auth/widgets/sign_in/otp_login_widget.dart';
...
class _SignInViewState extends State<SignInView> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_){
      ...
      bool isOtpActive = centralizeLoginSetup != null &&
          (CentralizeLoginHelper.getPreferredLoginMethod(centralizeLoginSetup, authController.isOtpViewEnable).type == CentralizeLoginType.otp
          || CentralizeLoginHelper.getPreferredLoginMethod(centralizeLoginSetup, authController.isOtpViewEnable).type == CentralizeLoginType.otpAndSocial);
  ...
  Widget build(BuildContext context) {
    return GetBuilder<AuthController>(builder: (authController) {
      final centralizeLoginSetup = Get.find<SplashController>().configModel?.centralizeLoginSetup
          ?? CentralizeLoginSetup(manualLoginStatus: true);
      return Form(
        key: _formKeyLogin,
    ...
    switch (centralizeLogin) {
      case CentralizeLoginType.otp:
        return OtpLoginWidget(
          ...
          onClickLoginButton: () {
            _otpLogin(Get.find<AuthController>(), _countryDialCode ?? '+966', CentralizeLoginType.otp);
          },
        );
  ...
  void _processOtpSuccessSetup(ResponseModel response, AuthController authController, String phone, String countryDialCode) async {
    ...
    // OTP was sent successfully — always navigate to verification screen
