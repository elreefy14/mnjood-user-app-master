...
import 'package:mnjood/features/auth/widgets/sign_in/existing_user_bottom_sheet.dart';
...
class SocialLoginWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    bool canAppleLogin = (configModel?.appleLogin?.isNotEmpty ?? false)
        && (configModel?.appleLogin?[0].status ?? false)
        && !GetPlatform.isAndroid && !GetPlatform.isWeb;

    bool canGoogleAndFacebookLogin = (configModel?.socialLogin?.isNotEmpty ?? false)
        && ((configModel?.socialLogin?[0].status ?? false)
        || ((configModel?.socialLogin?.length ?? 0) > 1 && (configModel?.socialLogin?[1].status ?? false)));
      ...
      return Column(
        ...
        children: [
          canGoogleAndFacebookLogin ? Column(children: [
            ...
            (configModel?.socialLogin?[0].status ?? false) ? Container(
              height: 50,
            ...
            SizedBox(height: (configModel?.socialLogin?[0].status ?? false) ? Dimensions.paddingSizeLarge : 0),

            ((configModel?.socialLogin?.length ?? 0) > 1 && (configModel?.socialLogin?[1].status ?? false)) ? Container(
              height: 50,
              padding: const EdgeInsets.all(1),
              decoration: BoxDecoration(
                color: Theme.of(context).cardColor,
            ...
            SizedBox(height: ((configModel?.socialLogin?.length ?? 0) > 1 && (configModel?.socialLogin?[1].status ?? false)) ? Dimensions.paddingSizeLarge : 0),
    ...
    return canGoogleAndFacebookLogin || canAppleLogin ? Column(children: [
      ...
      Row(mainAxisAlignment: MainAxisAlignment.center, children: [
        ...
        (configModel?.socialLogin?[0].status ?? false) ? InkWell(
          onTap: () => _googleLogin(googleSignIn),
          child: Container(
            height: 40,width: 40,
            padding: const EdgeInsets.all(1),
        ...
        ((configModel?.socialLogin?.length ?? 0) > 1 && (configModel?.socialLogin?[1].status ?? false)) ? Padding(
