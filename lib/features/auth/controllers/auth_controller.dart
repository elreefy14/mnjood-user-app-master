...
class AuthController extends GetxController implements GetxService {
  ...
  var countryDialCode= "+966";  // Saudi Arabia
  ...
  Future<ResponseModel> verifyOtp({required String phone, required String otp, bool alreadyInApp = false}) async {
    _isLoading = true;
    update();
    ResponseModel responseModel = await authServiceInterface.verifyOtp(phone: phone, otp: otp, alreadyInApp: alreadyInApp);
    _getUserAndCartData(responseModel);
    _isLoading = false;
    update();
    return responseModel;
  ...
  void _getUserAndCartData(ResponseModel responseModel) {
    // Only fetch user info and cart when fully authenticated (token is saved)
    // Token is only saved when all verification conditions are met
    if(responseModel.isSuccess && responseModel.authResponseModel != null && responseModel.authResponseModel!.isPhoneVerified!
      ...
      // Load cart after successful login - token is now set in API headers
  ...
  void initCountryCode({String? countryCode}){
    ...
    countryDialCode = countryCode ?? (country != null && country.isNotEmpty
        ? CountryCode.fromCountryCode(country).dialCode ?? "+966"
        : "+966");  // Default to Saudi Arabia
  }
