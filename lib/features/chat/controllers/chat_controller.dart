...
import 'package:mnjood/features/chat/domain/services/chat_service_interface.dart';
import 'package:mnjood/features/chat/enums/user_type_enum.dart';
import 'package:mnjood/helper/date_converter.dart';
import 'package:mnjood/helper/image_size_checker.dart';
...
class ChatController extends GetxController implements GetxService {
      ...
      if(index0 != -1) {
        if(sender) {
          _searchConversationModel!.conversations![index0]!.sender = User(
            ...
            imageFullUrl: null, // Use null so UI shows static logo
          );
        }else {
  ...
  required int? conversationID, required int? index}) async {
    ...
    if (messageModel != null) {
      ...
      if(index != null && _searchConversationModel != null && _messageModel?.messages?.isNotEmpty == true) {
      ...
      }else if(index != null && _conversationModel != null && _messageModel?.messages?.isNotEmpty == true) {
      ...
      if(_conversationModel != null && !_hasAdmin && _messageModel?.conversation != null && (_messageModel!.conversation!.senderType == UserType.admin.name || _messageModel!.conversation!.receiverType == UserType.admin.name)) {
        _conversationModel!.conversations!.add(_messageModel!.conversation);
        _hasAdmin = true;
      }
      if(Get.find<ProfileController>().userInfoModel?.userInfo == null && _messageModel?.conversation?.sender != null) {
  ...
  void _sortMessage(int? adminId) {
    ...
    if(adminId != null && _messageModel?.conversation != null) {
      _messageModel!.conversation!.receiver = User(
        id: 0, fName: Get.find<SplashController>().configModel?.businessName ?? 'Admin', lName: '',
        imageFullUrl: null, // Use null so UI shows static logo
      );
      getAdminMessages();
    }
