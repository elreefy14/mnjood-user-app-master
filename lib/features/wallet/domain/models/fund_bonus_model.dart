class FundBonusModel {
  ...
  FundBonusModel.fromJson(Map<String, dynamic> json) {
    ...
    minimumAddAmount = json['minimum_add_amount'] != null ? double.tryParse(json['minimum_add_amount'].toString()) : null;
    maximumBonusAmount = json['maximum_bonus_amount'] != null ? double.tryParse(json['maximum_bonus_amount'].toString()) : null;
    startDate = json['start_date'];
    endDate = json['end_date'];
    // V3 API returns status as string ("active"/"inactive"), V1 returns int
    var rawStatus = json['status'];
    if (rawStatus is int) {
      status = rawStatus;
    } else if (rawStatus is String) {
      status = (rawStatus == 'active' || rawStatus == '1') ? 1 : 0;
    }
