...
import 'package:mnjood/features/checkout/widgets/dine_in_date_select_bottom_sheet.dart';
...
class EstimatedArrivalTimeWidget extends StatelessWidget {
        ...
        InkWell(
          onTap: () {
            ...
            if(ResponsiveHelper.isDesktop(context)) {
              Get.dialog(Dialog(child: DineInDateSelectBottomSheet(restaurant: restaurant)));
            } else {
              showCustomBottomSheet(child: DineInDateSelectBottomSheet(restaurant: restaurant));
            }
          },
        ...
        InkWell(
          onTap: () async {
            ...
            if(checkoutController.selectedDineInDate == null) {
              ...
              final restaurant = checkoutController.restaurant;
              final dineInDuration = restaurant?.dineInBookingDuration ?? 0;
              final dineInTimeFormat = restaurant?.dineInBookingDurationTimeFormat ?? 'min';

              TimeOfDay time = _addHours(TimeOfDay.now(), dineInDuration, dineInTimeFormat);
              ...
              if (pickedTime != null && pickedTime != TimeOfDay.now()) {
                final selectedDineInDate = checkoutController.selectedDineInDate;
                if (selectedDineInDate == null) return;

                DateTime datetime = DateConverter.formattingDineInDateTime(pickedTime, selectedDineInDate);

                final timeSlots = checkoutController.timeSlots;
                bool inTime = isInDineInSchedule(timeSlots, datetime, pickedTime, dineInTimeFormat, dineInDuration).$1;
                String? message = isInDineInSchedule(timeSlots, datetime, pickedTime, dineInTimeFormat, dineInDuration).$2;
          ...
          child: Stack(clipBehavior: Clip.none, children: [
            ...
            Container(
              ...
              child: Row(children: [
                ...
                Icon(HeroiconsOutline.clock, color: Theme.of(context).disabledColor),
                const SizedBox(width: Dimensions.paddingSizeSmall),
              ]),
            ),
  ...
  (bool, String?) isInDineInSchedule(List<TimeSlotModel>? timeSlots, DateTime datetime, TimeOfDay pickTime, String dineInTimeType, int duration) {
    if (timeSlots == null || timeSlots.isEmpty) return (false, null);
    ...
    for (var v in timeSlots) {
      final startTime = v.startTime;
      final endTime = v.endTime;
