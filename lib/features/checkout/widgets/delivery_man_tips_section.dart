...
import 'package:mnjood/features/checkout/widgets/tips_widget.dart';
...
class _DeliveryManTipsSectionState extends State<DeliveryManTipsSection> {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        (!widget.checkoutController.subscriptionOrder && !widget.takeAway && !isDineIn && (Get.find<SplashController>().configModel?.dmTipsStatus ?? 0) == 1) ? Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Icon(HeroiconsOutline.heart, size: 20, color: Theme.of(context).primaryColor),
              const SizedBox(width: Dimensions.paddingSizeExtraSmall),
              Text('delivery_man_tips'.tr, style: robotoSemiBold),
                      ...
                      try{
                        if(double.parse(value) >= 0){
                          if(Get.find<AuthController>().isLoggedIn()) {
                            ...
                            if(walletBalance < total && widget.checkoutController.paymentMethodIndex == 1){
                              widget.checkoutController.checkBalanceStatus(total);
                              canCheckSmall = true;
                            } else if(walletBalance > total && canCheckSmall && widget.checkoutController.isPartialPay){
                              widget.checkoutController.checkBalanceStatus(total);
                            }
                          } else {
        ...
        SizedBox(height: (!widget.takeAway && !isDineIn && (Get.find<SplashController>().configModel?.dmTipsStatus ?? 0) == 1)
