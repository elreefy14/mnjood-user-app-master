    ...
    return InkWell(
      onTap: () async {
        DateTimeRange? range = await showDateRangePicker(
          context: context,
          firstDate: isPause ? DateTime.now().add(Duration(days: 1)) : DateTime.now(),
          lastDate: isPause ? DateTime.parse(Get.find<OrderController>().trackModel!.subscription!.endAt!) : DateTime.now().add(const Duration(days: 365)),
      ...
      child: Container(
        ...
        child: Row(children: [
          ...
          Icon(HeroiconsOutline.calendar, size: 24, color: range != null ? Theme.of(context).primaryColor : Theme.of(context).disabledColor),
        ]),
      ),
    );
  }
