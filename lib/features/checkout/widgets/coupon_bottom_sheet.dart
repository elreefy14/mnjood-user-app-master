...
import 'package:mnjood/features/coupon/widgets/coupon_card_widget.dart';
...
class _CouponBottomSheetState extends State<CouponBottomSheet> {
  ...
  void initState() {
    ...
    final customerCouponModel = Get.find<CouponController>().customerCouponModel;
    final availableCoupons = customerCouponModel?.available;
    ...
    final unavailableCoupons = customerCouponModel?.unavailable;
    if(unavailableCoupons != null && unavailableCoupons.isNotEmpty) {
      _unavailableCouponList = [];
      _unavailableToolTipControllerList = [];
      for (var coupon in unavailableCoupons) {
        if(widget.deliveryCharge == 0 && coupon.couponType != 'free_delivery') {
          ...
          return Column(
            children: [
              ...
              Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                ...
                IconButton(
                  onPressed: ()=> Get.back(),
                  icon: Icon(HeroiconsOutline.xMark, color: Theme.of(context).disabledColor),
                ),
              ]),
                ...
                child: Row(children: [
                    ...
                    child: SizedBox(
                      ...
                      child: TextField(
                        ...
                        decoration: InputDecoration(
                            ...
                            prefixIcon: Icon(HeroiconsOutline.tag, color: Theme.of(context).primaryColor)
                        ),
                      ),
                    ),
                  ),
                  InkWell(
                    onTap: () {
                      ...
                      if((couponController.discount ?? 0) < 1 && !couponController.freeDelivery) {
                        if(couponCode.isNotEmpty && !couponController.isLoading) {
                          final restaurantId = Get.find<RestaurantController>().restaurant?.id;
                          if (restaurantId != null) {
                            couponController.applyCoupon(couponCode, (widget.price-widget.discount)+widget.addOns, widget.deliveryCharge,
                                widget.charge, totalPrice, restaurantId).then((discount) {
                        ...
                        totalPrice = totalPrice + (couponController.discount ?? 0);
                    ...
                    child: Container(
                      height: 45, width: ((couponController.discount ?? 0) <= 0 && !couponController.freeDelivery) ? 100 : 50,
                      alignment: Alignment.center,
                      margin: const EdgeInsets.all(Dimensions.paddingSizeExtraSmall),
                      decoration: BoxDecoration(
                        color: ((couponController.discount ?? 0) <= 0 && !couponController.freeDelivery) ? Theme.of(context).primaryColor : Colors.transparent,
                        borderRadius: BorderRadius.circular(Dimensions.radiusDefault),
                      ),
                      child: ((couponController.discount ?? 0) <= 0 && !couponController.freeDelivery) ? !couponController.isLoading ? Text(
                        'apply'.tr,
                        ...
                        child: CircularProgressIndicator(valueColor: AlwaysStoppedAnimation<Color>(Colors.white)),
                          ...
                          : Icon(HeroiconsOutline.xMark, color: Theme.of(context).colorScheme.error),
                    ),
                  ),
                ]),
              ),
                                ...
                                if(widget.checkoutController.couponController.text.isNotEmpty && !couponController.isLoading) {
                                  ...
                                  if (restaurantId != null) {
                                    couponController.applyCoupon(widget.checkoutController.couponController.text, (widget.price-widget.discount)+widget.addOns, widget.deliveryCharge,
                                      widget.charge, totalPrice, restaurantId, hideBottomSheet: true).then((discount) {
