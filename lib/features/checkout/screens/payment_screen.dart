...
import 'package:mnjood/features/wallet/widgets/fund_payment_dialog_widget.dart';
...
class PaymentScreen extends StatefulWidget {
  ...
  final int? packageId;
  final String? payFirstUrl;
  const PaymentScreen({super.key, required this.orderModel, required this.paymentMethod, this.addFundUrl, this.subscriptionUrl,
    required this.guestId, required this.contactNumber, this.restaurantId, this.packageId, this.payFirstUrl});
...
class PaymentScreenState extends State<PaymentScreen> {
  ...
  void initState() {
    ...
    if(widget.payFirstUrl != null && widget.payFirstUrl!.isNotEmpty) {
      // PAY-FIRST: use the pre-initialized payment URL
      selectedUrl = widget.payFirstUrl!;
    } else if((widget.addFundUrl == null || widget.addFundUrl!.isEmpty) && (widget.subscriptionUrl == null || widget.subscriptionUrl!.isEmpty)) {
      selectedUrl = '${AppConstants.baseUrl}/payment-mobile?customer_id=${widget.orderModel.userId == 0 ? widget.guestId : widget.orderModel.userId}'
          '&order_id=${widget.orderModel.id}&payment_method=${widget.paymentMethod}'
          '&payment_platform=app&callback=${Uri.encodeComponent('${AppConstants.baseUrl}/payment-success')}';
  ...
  void _initData() async {
    ...
    if(widget.addFundUrl == null || widget.addFundUrl!.isEmpty){
      ...
      if (zoneDataList != null && zoneDataList.isNotEmpty) {
    ...
    browser = MyInAppBrowser(orderID: widget.orderModel.id.toString(), orderAmount: widget.orderModel.orderAmount, maxCodOrderAmount: maxCodOrderAmount, addFundUrl: widget.addFundUrl,
        ...
        payFirstUrl: widget.payFirstUrl);
...
class MyInAppBrowser extends InAppBrowser {
  ...
  MyInAppBrowser({required this.orderID, required this.orderAmount, required this.maxCodOrderAmount, this.contactNumber, super.windowId,
    super.initialUserScripts, this.addFundUrl, this.subscriptionUrl, this.restaurantId, this.packageId, this.isDeliveryOrder = false, this.payFirstUrl});
  ...
  void onExit() {
    if(_canRedirect) {
      if (payFirstUrl != null && payFirstUrl!.isNotEmpty) {
        // PAY-FIRST: no order created, just clean up
        Get.find<CheckoutController>().clearPendingOrderData();
        Get.back();
        showCustomSnackBar('payment_cancelled'.tr);
      } else if((addFundUrl == null || addFundUrl!.isEmpty) && subscriptionUrl == '' && subscriptionUrl!.isEmpty){
  ...
  void _orderPaymentDoneDecision(bool isSuccess, bool isFailed, bool isCancel) {
    ...
    if (isSuccess) {
      if (ctrl.pendingPlaceOrderBody != null) {
        // PAY-FIRST: create order now with webview session proof
        ctrl.placeOrderAfterPayment(webviewSessionId: orderID);
        // placeOrderAfterPayment handles navigation on success
      } else {
        // LEGACY: send restaurant notification — payment has been confirmed
        ctrl.sendOrderNotification(orderID);
      ...
      if (ctrl.pendingPlaceOrderBody != null) {
        // PAY-FIRST: no order created, clear pending and go back
        ctrl.clearPendingOrderData();
        Get.back();
        showCustomSnackBar('payment_cancelled'.tr);
      } else {
