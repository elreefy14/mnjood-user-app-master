...
class PredictionModel {
  ...
  PredictionModel.fromJson(Map<String, dynamic> json) {
    // Handle V3 flat format (from our backend)
    if (json['place_id'] != null || json['description'] != null) {
      description = json['description'];
      placeId = json['place_id'];
      id = json['place_id'];
      reference = json['description'];
      // Handle structured format fields
      if (json['main_text'] != null) {
        structuredFormat = StructuredFormat(
          mainText: MainText(text: json['main_text']),
          secondaryText: SecondaryText(text: json['secondary_text']),
        );
      }
      return;
    }

    // Handle original nested format (Google Places API format)
