...
import 'package:mnjood/features/review/widgets/deliver_man_review_widget.dart';
import 'package:mnjood/features/review/widgets/product_review_widget.dart';
...
class RateReviewScreenState extends State<RateReviewScreen> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    final tabLength = widget.rateReviewModel.deliveryMan == null ? 1 : 2;
    _tabController = TabController(length: tabLength, initialIndex: 0, vsync: this);
    if (widget.rateReviewModel.orderDetailsList != null && widget.rateReviewModel.orderDetailsList!.isNotEmpty) {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<ReviewController>(builder: (reviewController) {
      return PopScope(
        canPop: !reviewController.isLoading,
        child: Scaffold(
          resizeToAvoidBottomInset: true,
          appBar: CustomAppBarWidget(title: 'rate_review'.tr),
          endDrawer: const MenuDrawerWidget(), endDrawerEnableOpenDragGesture: false,
          body: reviewController.hasError
              ? Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Text('rating_error'.tr, style: robotoMedium.copyWith(fontSize: Dimensions.fontSizeLarge)),
                      const SizedBox(height: Dimensions.paddingSizeLarge),
                      ElevatedButton(
                        onPressed: () {
                          if (widget.rateReviewModel.orderDetailsList != null) {
                            reviewController.initRatingData(widget.rateReviewModel.orderDetailsList!);
                          }
                        },
                        child: Text('retry'.tr),
                      ),
