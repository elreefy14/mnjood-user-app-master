...
import 'package:mnjood/helper/splash_route_helper.dart';
...
class SplashController extends GetxController implements GetxService {
  ...
  static const int _maxConfigRetries = 3;
  ...
  void _handleConfigResponse(Response response, bool handleMaintenanceMode, bool fromMainFunction, bool fromDemoReset, {required NotificationBodyModel? notificationBody, required DeepLinkBody? linkBody}) {
    if(response.statusCode == 200) {
      ...
      if(_configModel != null) {
        _configRetryCount = 0; // Reset on success
        if(!GetPlatform.isWeb){
          bool isMaintenanceMode = _configModel!.maintenanceMode ?? false;
          bool isInMaintenance = MaintenanceHelper.isMaintenanceEnable();
        ...
        // Config parsing failed, retry
        _retryConfigLoad(handleMaintenanceMode, fromMainFunction, fromDemoReset, notificationBody: notificationBody, linkBody: linkBody);
      }
    } else {
      if(response.statusText == ApiClient.noInternetMessage) {
        _hasConnection = false;
  ...
  void _retryConfigLoad(bool handleMaintenanceMode, bool fromMainFunction, bool fromDemoReset, {required NotificationBodyModel? notificationBody, required DeepLinkBody? linkBody}) {
    if (_configRetryCount < _maxConfigRetries) {
      _configRetryCount++;
      print('Config load failed, retrying... (attempt $_configRetryCount/$_maxConfigRetries)');
      Future.delayed(const Duration(seconds: 2), () {
        getConfigData(handleMaintenanceMode: handleMaintenanceMode, source: DataSourceEnum.client, notificationBody: notificationBody, fromMainFunction: fromMainFunction, fromDemoReset: fromDemoReset);
      });
    } else {
      print('Config load failed after $_maxConfigRetries attempts');
      _hasConnection = false;
      update();
    }
  }
