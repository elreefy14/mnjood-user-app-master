...
class OrderController extends GetxController implements GetxService {
  ...
  bool _isSubstitutionLoading = false;
  bool get isSubstitutionLoading => _isSubstitutionLoading;
  ...
  Future<void> getRunningOrders(int offset, {bool notify = true, int limit = 100}) async {
    ...
    if (!_runningOffsetList.contains(offset)) {
      ...
      if (paginatedOrderModel != null) {
        ...
        // Handle null response - set empty list to stop loading state
        if (offset == 1) {
          _runningOrderList = [];
          _runningPageSize = 0;
        }
  ...
  Future<void> getRunningSubscriptionOrders(int offset, {bool notify = true}) async {
    ...
    if (!_runningSubscriptionOffsetList.contains(offset)) {
      ...
      if (paginatedOrderModel != null) {
        ...
        if (offset == 1) {
          _runningSubscriptionOrderList = [];
          _runningSubscriptionPageSize = 0;
        }
        _runningSubscriptionPaginate = false;
        update();
      }
  ...
  Future<bool> timerTrackOrder(String orderID, {String? contactNumber}) async {
    ...
    if(orderModel != null) {
      // Save previous driver position for bearing calculation
      if (_trackModel?.deliveryMan != null) {
        _prevDriverLat = double.tryParse(_trackModel!.deliveryMan!.lat ?? '0');
        _prevDriverLng = double.tryParse(_trackModel!.deliveryMan!.lng ?? '0');
      }
      _trackModel = orderModel;

      // Calculate driver bearing from previous to current position
      if (_prevDriverLat != null && _prevDriverLng != null && orderModel.deliveryMan != null) {
        final curLat = double.tryParse(orderModel.deliveryMan!.lat ?? '0') ?? 0;
        final curLng = double.tryParse(orderModel.deliveryMan!.lng ?? '0') ?? 0;
        if (_prevDriverLat != curLat || _prevDriverLng != curLng) {
          _driverBearing = _calculateBearing(_prevDriverLat!, _prevDriverLng!, curLat, curLng);
        }
      }
    }
    update();
  ...
  double _calculateBearing(double lat1, double lng1, double lat2, double lng2) {
    ...
    final y = math.cos(lat1Rad) * math.sin(lat2Rad) - math.sin(lat1Rad) * math.cos(lat2Rad) * math.cos(dLng);
  ...
  Future<void> getSubstitutionProposals(int orderId) async {
    _isSubstitutionLoading = true;
    update();
    _substitutionProposals = await orderServiceInterface.getSubstitutionProposals(orderId);
    _isSubstitutionLoading = false;
    update();
  }

  Future<bool> respondToSubstitution(int proposalId, String action) async {
    _isSubstitutionLoading = true;
    update();
    ResponseModel response = await orderServiceInterface.respondToSubstitution(proposalId, action);
    if (response.isSuccess && _substitutionProposals != null) {
      final index = _substitutionProposals!.indexWhere((p) => p.id == proposalId);
      if (index != -1) {
        _substitutionProposals![index].status = action == 'accept' ? 'accepted' : 'rejected';
      }
    }
    _isSubstitutionLoading = false;
    update();
  ...
  List<SubstitutionProposal> get pendingSubstitutions {
    if (_substitutionProposals == null) return [];
    return _substitutionProposals!.where((p) => p.status == 'pending').toList();
  }

  Future<bool> cancelOrder(int? orderID, String? cancelReason, {int? reasonId, String? customReason}) async {
    _isCancelLoading = true;
    update();
    ResponseModel responseModel = await orderServiceInterface.cancelOrder(orderID.toString(), cancelReason, reasonId: reasonId, customReason: customReason);
  ...
  Future<void> cancelOrderSilently(int orderId) async {
    await orderServiceInterface.cancelOrder(orderId.toString(), 'edit_order');
    OrderModel? orderModel = orderServiceInterface.findOrder(_runningOrderList, orderId);
    if (orderModel != null) {
      _runningOrderList?.remove(orderModel);
      update();
