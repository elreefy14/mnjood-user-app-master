...
import 'package:mnjood/features/order/widgets/cancellation_bottom_sheet.dart';
import 'package:mnjood/features/order/widgets/subscription_pause_dialog.dart';
    ...
    return Column(children: [
      !orderController.showCancelled ? Container(
        ...
        decoration: !isDesktop ? BoxDecoration(
          ...
          boxShadow: [BoxShadow(color: Theme.of(context).disabledColor.withValues(alpha: 0.1), blurRadius: 6, offset: const Offset(0, -2))],
        ) : null,
        child: Center(
          child: SizedBox(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              (!offlinePay && pending && order.paymentStatus == 'unpaid' && digitalPay) ? Row(children: [
                Expanded(
                  child: CustomButtonWidget(
                    buttonText: 'pay_now'.tr,
                    onPressed: () {
                      final activePaymentList = Get.find<SplashController>().configModel?.activePaymentMethodList;
                      if (activePaymentList != null && activePaymentList.isNotEmpty) {
                        Get.toNamed(RouteHelper.getPaymentRoute(
                          order, activePaymentList.first.getWay,
                          guestId: Get.find<AuthController>().isLoggedIn() ? '' : Get.find<AuthController>().getGuestId(),
                ...
                (Get.find<SplashController>().configModel?.cashOnDelivery ?? false) ? Expanded(
                  child: CustomButtonWidget(
              ...
              (!offlinePay && pending && order.paymentStatus == 'unpaid' && digitalPay) ? const SizedBox(height: Dimensions.paddingSizeSmall) : const SizedBox(),
                ...
                (subscription ? (order.subscription!.status == 'active' || order.subscription!.status == 'paused') : (pending)) ? Expanded(child: CustomButtonWidget(
                  ...
                  onPressed: () {
                    if(subscription) {
                      ...
                      showCancellationBottomSheet(orderId: order.id);
                    }
                  },
                )) : const SizedBox(),
      ...
      (orderController.orderDetails != null && orderController.orderDetails!.isNotEmpty) &&
      ((!subscription && delivered && orderController.orderDetails![0].itemCampaignId == null) || (!subscription && (Get.find<SplashController>().configModel?.repeatOrderOption ?? false) && (delivered || cancelled || order.orderStatus == 'failed' || order.orderStatus == 'refund_request_canceled')))
          ...
          child: !orderController.isLoading ? Get.find<AuthController>().isLoggedIn() ? Padding(
            ...
            child: Row(
              children: [
                ...
                !subscription && (Get.find<SplashController>().configModel?.repeatOrderOption ?? false) && (delivered || cancelled || order.orderStatus == 'failed' || order.orderStatus == 'refund_request_canceled')
                ? (orderController.orderDetails != null && orderController.orderDetails!.isNotEmpty && orderController.orderDetails![0].itemCampaignId == null) ? Expanded(
                  child: CustomButtonWidget(
      ...
      (!offlinePay && (order.orderStatus == 'failed' || cancelled) && !cod && (Get.find<SplashController>().configModel?.cashOnDelivery ?? false)) ? Center(
        child: Container(
          width: Dimensions.webMaxWidth,
