...
import 'package:mnjood/common/enums/payment_type.dart';
...
import 'package:mnjood/features/order/widgets/log_bottom_sheet_widget.dart';
import 'package:mnjood/features/order/widgets/offline_info_edit_dialog.dart';
...
import 'package:mnjood/features/review/widgets/review_dialog_widget.dart';
...
import 'package:mnjood/features/chat/widgets/image_dialog_widget.dart';
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  Center(child: Padding(
                    ...
                    child: order.orderStatus == 'pending' ? Text(
                      ...
                      Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        Text(
                          () {
                            int remaining = DateConverter.differenceInMinute(order.restaurant!.deliveryTime, order.createdAt, order.processingTime, order.scheduleAt, fromDineIn: true, processing: order.processing);
                      ...
                      Row(mainAxisSize: MainAxisSize.min, children: [
                        ...
                        Text(
                          () {
                            int remaining = DateConverter.differenceInMinute(null, order.createdAt, null, order.scheduleAt);
                            if (remaining <= 0) return '0 ${'min'.tr}';
                            if (remaining < 5) return '$remaining ${'min'.tr}';
                            return DateConverter.convertMinutesToDayHourMinute(remaining);
                          }(),
                          style: robotoBold.copyWith(fontSize: Dimensions.fontSizeExtraLarge), textDirection: TextDirection.ltr,
                        ),
                        // const SizedBox(width: Dimensions.paddingSizeExtraSmall),
          ...
          isDineIn ? Dimensions.paddingSizeSmall : 0 : 0 : 0),
          ...
          CustomCard(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              subscription && order.createdAt != null ? Row(children: [
                ...
                !isDineIn && order.createdAt != null ? Row(children: [
                  const Icon(HeroiconsOutline.clock, size: 17),
                  const SizedBox(width: Dimensions.paddingSizeExtraSmall),
                  Text(
                ...
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: (subscription ? order.subscription?.status == 'canceled' || order.subscription?.status == 'expired' : (order.orderStatus == 'failed' || cancelled || order.orderStatus == 'refund_request_canceled'))
                      ? const Color(0xFFE84D4F).withValues(alpha: 0.1) : order.orderStatus == 'refund_requested' ? Theme.of(context).primaryColor.withValues(alpha: 0.1) : const Color(0xFF2ECC71).withValues(alpha: 0.1),
                    borderRadius: BorderRadius.circular(Dimensions.radiusSmall),
                  ),
                  child: Text(
                    delivered && order.delivered != null ? '${'delivered_at'.tr} ${DateConverter.dateTimeStringToDateTime(order.delivered!)}'
                        : subscription ? (order.subscription?.status ?? 'pending').tr : (order.orderStatus ?? 'pending').tr,
                    style: robotoBold.copyWith(
                      fontSize: Dimensions.fontSizeSmall,
                      color: (subscription ? order.subscription?.status == 'canceled' || order.subscription?.status == 'expired' : (order.orderStatus == 'failed' || cancelled || order.orderStatus == 'refund_request_canceled'))
                        ? const Color(0xFFE84D4F) : order.orderStatus == 'refund_requested' ? Theme.of(context).primaryColor : const Color(0xFF2ECC71),
                    ),
                  ),
                ),
              ...
              (Get.find<SplashController>().configModel?.orderDeliveryVerification ?? false) && order.otp != null ? Row(children: [
              ...
              (Get.find<SplashController>().configModel?.orderDeliveryVerification ?? false) && order.otp != null ? const Divider(height: Dimensions.paddingSizeLarge) : const SizedBox(),

              Row(children: [
                Text((order.orderType ?? 'delivery').tr, style: robotoRegular),
                const Expanded(child: SizedBox()),
                Container(
              ...
              subscription ? const SizedBox() : Padding(
                ...
                child: Row(children: [
                  ...
                  const SizedBox(width: Dimensions.paddingSizeExtraSmall),
                  Text(
                    delivered && order.delivered != null ? '${isDineIn ? 'served_at'.tr : 'delivered_at'.tr} ${DateConverter.dateTimeStringToDateTime(order.delivered!)}'
                        : subscription ? (order.subscription?.status ?? 'pending').tr
                        ...
                        : 'served'.tr : (order.orderStatus ?? 'pending').tr,
                    style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeSmall),
                  ),
                ]),
              ),

              !isDineIn && order.cutlery != null ? Column(children: [
     ...
     /* isDesktop && !isDineIn ? Padding(
      ...
      // Store/Restaurant details section removed - only keeping driver info
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(
            ...
            children: [
              ...
              Text((order.paymentStatus ?? 'unpaid').tr, style: robotoRegular.copyWith(
          ...
          Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            !isDesktop ? Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
              ...
              order.paymentMethod == 'offline_payment' || (order.paymentMethod == 'partial_payment' && orderController.trackModel?.offlinePayment != null) ? Text(
                ...
                style: robotoMedium.copyWith(color: (orderController.trackModel?.offlinePayment?.data?.status == 'denied') ? const Color(0xFFE84D4F) : Theme.of(context).primaryColor),
              ) : const SizedBox(),
            ]) : const SizedBox(),
  ...
  return ListTileTheme(
    ...
    child: Theme(
      ...
      child: ExpansionTile(
        ...
        trailing: Icon(orderController.isExpanded ? HeroiconsOutline.chevronDown : HeroiconsOutline.chevronUp, size: 18),
        ...
        children: [
          ...
          orderController.trackModel?.offlinePayment != null ? ListView.builder(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
            itemCount: orderController.trackModel?.offlinePayment?.methodFields?.length ?? 0,
            itemBuilder: (context, index){
              final methodField = orderController.trackModel?.offlinePayment?.methodFields?[index];
              return Padding(
                padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeExtraSmall),
                child: Row(children: [
                  Text('${methodField?.inputName?.toString().replaceAll('_', ' ') ?? ''} : ', style: robotoRegular),
                  Text('${methodField?.inputData ?? ''}', style: robotoRegular),
                ]),
              );
            },
          ...
          orderController.trackModel?.offlinePayment != null ? Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            ...
            (ongoing && orderController.trackModel?.offlinePayment?.data?.status != 'verified') ? InkWell(
              onTap: (){
                if (orderController.trackModel?.offlinePayment != null && orderController.trackModel?.id != null) {
          ...
          orderController.trackModel?.offlinePayment != null ? ListView.builder(
            shrinkWrap: true,
            physics: const NeverScrollableScrollPhysics(),
            itemCount: orderController.trackModel?.offlinePayment?.input?.length ?? 0,
            itemBuilder: (context, index){
              final data = orderController.trackModel?.offlinePayment?.input?[index];
              return Padding(
                padding: const EdgeInsets.only(bottom: Dimensions.paddingSizeExtraSmall),
                child: Row(children: [
    ...
    return Dialog(
      ...
      child: Stack(children: [
        ...
        Positioned(top: 0, right: 0, child: IconButton(
          splashRadius: 5,
          onPressed: () => Get.back(),
          icon: const Icon(HeroiconsOutline.xCircle, color: Color(0xFFE84D4F)),
        )),
