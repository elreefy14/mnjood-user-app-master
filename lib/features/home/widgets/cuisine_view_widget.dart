...
import 'package:mnjood/features/home/widgets/cuisine_card_widget.dart';
...
class CuisineViewWidget extends StatelessWidget {
  ...
  // Translate cuisine name from API to localized string
  String _getTranslatedCuisineName(String? name) {
    if (name == null || name.isEmpty) return '';
    final key = 'cuisine_${name.toLowerCase()}';
    final translated = key.tr;
    // If translation exists (key != translated), use it; otherwise use original
  ...
  Widget build(BuildContext context) {
    return GetBuilder<CuisineController>(builder: (cuisineController) {
        return (cuisineController.cuisineModel != null && (cuisineController.cuisineModel!.cuisines?.isEmpty ?? true)) ? const SizedBox() : Container(
            ...
            children: [
              Padding(padding: const EdgeInsets.all(Dimensions.paddingSizeLarge),
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('cuisine'.tr, style: sectionTitleStyle),
                ...
                itemBuilder: (context, index) {
                  return CustomInkWellWidget(
                    ...
                    child: CuisineCardWidget(
                      ...
                      name: _getTranslatedCuisineName(cuisineController.cuisineModel!.cuisines![index].name),
                    ),
                  );
