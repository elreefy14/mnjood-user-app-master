...
class ProductService implements ProductServiceInterface {
  ...
  @override
  Future<Product?> getProductDetails({required int id, required bool isCampaign, int? vendorId, String? businessType}) async {
    return await productRepositoryInterface.get(id.toString(), isCampaign: isCampaign, vendorId: vendorId, businessType: businessType);
  }

  @override
  Future<List<Product>?> getPharmacyProducts({DataSourceEnum? source}) async {
    return await productRepositoryInterface.getPharmacyProducts(source: source);
  }
  ...
  List<bool> initializeCartAddonActiveList(Product? product, List<AddOn>? addOnIds) {
    ...
    if(addOnIds != null) {
      ...
      // Null-safe product addOns access
      final productAddOns = product?.addOns;
  ...
  List<int?> initializeCartAddonQuantityList(Product? product, List<AddOn>? addOnIds) {
    ...
    if(addOnIds != null) {
      ...
      if (productAddOns != null) {
        for (var addOn in productAddOns) {
          if(addOnIdList.contains(addOn.id)) {
            final index = addOnIdList.indexOf(addOn.id);
            addOnQtyList.add(index >= 0 && index < addOnIds.length ? addOnIds[index].quantity : 1);
          }else {
            addOnQtyList.add(1);
          }
        }
  ...
  List<List<bool?>> initializeSelectedVariation(List<Variation>? variations) {
    ...
    if(variations != null){
      for(int index=0; index<variations.length; index++){
        ...
        // Null-safe variationValues access
  ...
  List<bool> initializeAddonActiveList(List<AddOns>? addOns) {
    List<bool> addOnActiveList = [];
    if (addOns == null || addOns.isEmpty) return addOnActiveList;
  ...
  List<int?> initializeAddonQuantityList(List<AddOns>? addOns) {
    List<int?> addOnQtyList = [];
    if (addOns == null || addOns.isEmpty) return addOnQtyList;
    for (var addOn in addOns) {
      debugPrint('$addOn');
      addOnQtyList.add(1);
    }
  ...
  @override
  int setQuantity(bool isIncrement, int? cartQuantityLimit, int? maxQtyPerUser, int quantity, List<List<bool?>> selectedVariations, List<List<int?>> variationsStock, String? stockType, int? itemStock, bool isCampaign) {
    int qty = quantity;
    if (isIncrement) {
      qty = _quantityLimitCheck(selectedVariations, variationsStock, cartQuantityLimit, maxQtyPerUser, quantity, stockType, itemStock, isCampaign);
    } else {
      qty = qty - 1;
    }
    return qty;
  ...
  int _quantityLimitCheck(List<List<bool?>> selectedVariations, List<List<int?>> variationsStock, int? cartQuantityLimit, int? maxQtyPerUser, int quantity, String? stockType, int? itemStock, bool isCampaign) {
    int qty = quantity;
    int? minimumStock;
