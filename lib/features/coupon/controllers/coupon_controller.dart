...
import 'package:mnjood/features/coupon/domain/services/coupon_service_interface.dart';
...
class CouponController extends GetxController implements GetxService {
  ...
  Future<void> getRestaurantCouponList({required int restaurantId, String? vendorType}) async {
    _couponList = await couponServiceInterface.getRestaurantCouponList(restaurantId: restaurantId, vendorType: vendorType);
    update();
  }
  ...
  Future<double?> applyCoupon(String coupon, double order, double deliveryCharge, double charge, double total, int? restaurantID, {bool hideBottomSheet = false}) async {
    ...
    if (response.statusCode == 200) {
      ...
      _coupon = CouponModel.fromJson(data);
