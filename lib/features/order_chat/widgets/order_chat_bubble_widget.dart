...
class OrderChatBubbleWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (isBot) {
      return _buildCenterBubble(context, isBot: true);
    }
    if (isSystem) {
      return _buildCenterBubble(context, isBot: false);
    }
    if (isOwn) {
      return _buildOwnBubble(context);
    }
        ...
        children: [
          if (message.senderBadge != null)
            Padding(
              ...
              child: Text(
                '${message.senderName ?? ''} (${message.senderBadge})',
                style: robotoRegular.copyWith(fontSize: Dimensions.fontSizeExtraSmall, color: Theme.of(context).disabledColor),
              ),
            ),
          Container(
