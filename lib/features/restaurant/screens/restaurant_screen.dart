...
import 'package:mnjood/features/restaurant/widgets/pharmacy_supermarket_layout_widget.dart';
import 'package:mnjood/features/restaurant/widgets/restaurant_info_section_widget.dart';
import 'package:mnjood/features/restaurant/widgets/restaurant_screen_shimmer_widget.dart';
...
class RestaurantScreen extends StatefulWidget {
  ...
  final String? businessType; // Explicit businessType override (for search results navigation)
  const RestaurantScreen({super.key, required this.restaurant, this.slug = '', this.fromDineIn = false, this.businessType});
...
class _RestaurantScreenState extends State<RestaurantScreen> {
  ...
  Future<void> _initDataCall() async {
    ...
    // Use explicit businessType if provided, otherwise fall back to restaurant's businessType
    final effectiveBusinessType = widget.businessType ?? widget.restaurant?.businessType;
    ...
    final restaurantId = restaurant?.id ?? widget.restaurant?.id;
    // Use vendorId for product-related API calls (important for supermarkets/pharmacies)
    final vendorIdForProducts = restaurant?.vendorId ?? restaurantId;
    if (restaurantId != null && vendorIdForProducts != null) {
      // Use effectiveBusinessType which prioritizes explicit businessType parameter
      final actualBusinessType = effectiveBusinessType ?? restaurant?.businessType;
      Get.find<CouponController>().getRestaurantCouponList(restaurantId: restaurantId, vendorType: actualBusinessType);
      Get.find<RestaurantController>().getRestaurantRecommendedItemList(vendorIdForProducts, false, businessType: actualBusinessType);
      Get.find<RestaurantController>().getRestaurantProductList(vendorIdForProducts, 1, 'all', false, businessType: actualBusinessType);
      // Fetch vendor banners for supermarkets and pharmacies
      if (actualBusinessType == 'supermarket' || actualBusinessType == 'pharmacy') {
        Get.find<RestaurantController>().getVendorBanners(vendorIdForProducts);
      }
      // Fetch categories after restaurant is loaded
      Get.find<RestaurantController>().setCategoryList(
        vendorId: vendorIdForProducts,
    ...
    return Scaffold(
      ...
      body: GetBuilder<RestaurantController>(builder: (restController) {
        return GetBuilder<CouponController>(builder: (couponController) {
            ...
            // Use new layout for pharmacy and supermarket
            if (restaurant != null &&
                restaurant.name != null &&
                (restaurant.businessType == 'pharmacy' || restaurant.businessType == 'supermarket')) {
              return PharmacySupermarketLayoutWidget(
                restaurant: restaurant,
            ...
            return (restController.restaurant != null && restController.restaurant!.name != null) ? CustomScrollView(
              ...
              slivers: [
                  ...
                  child: Column(children: [
                    restaurant.discount != null ? Container(
                      ...
                      child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                        ...
                        SizedBox(height: ((restaurant.discount?.minPurchase ?? 0) != 0 || (restaurant.discount?.maxDiscount ?? 0) != 0) ? 5 : 0),

                        (restaurant.discount?.minPurchase ?? 0) != 0 ? Text(
                        ...
                        (restaurant.discount?.maxDiscount ?? 0) != 0 ? Text(
                    ...
                    SizedBox(height: ((restaurant.announcementActive ?? false) && restaurant.announcementMessage != null) ? 0 : Dimensions.paddingSizeSmall),

                    ResponsiveHelper.isMobile(context) ? ((restaurant.announcementActive ?? false) && restaurant.announcementMessage != null) ? Container(
                    ...
                    // Vendor Banner Carousel (for supermarkets and pharmacies)
                    if (restaurant.businessType == 'supermarket' || restaurant.businessType == 'pharmacy')
                      VendorBannerCarouselWidget(vendorId: restaurant.vendorId ?? restaurant.id ?? 0),

                    (restController.recommendedProductModel?.products?.isNotEmpty ?? false) ? Container(
                          ...
                          child: Row(children: [
                            ...
                            ArrowIconButtonWidget(
                              onTap: () => Get.toNamed(RouteHelper.getPopularFoodRoute(false, fromIsRestaurantFood: true, restaurantId: widget.restaurant?.id
                                  ?? Get.find<RestaurantController>().restaurant?.id ?? 0)),
                            ),
                          ]),
                        ),
                            ...
                            itemBuilder: (context, index) {
                              return Padding(
                                ...
                                child: ItemCardWidget(
                                  ...
                                  vendorId: restController.restaurant?.id,
                                ),
                              );
                            },
                          ),
                    ...
                    // Shop by Category (for supermarkets and pharmacies)
                    if (restaurant.businessType == 'supermarket' || restaurant.businessType == 'pharmacy')
                      ShopByCategoryWidget(vendorId: restaurant.vendorId ?? restaurant.id ?? 0),

                    // Mid-Page Hot Deals/Offers (for supermarkets - between categories and products)
                    if (restaurant.businessType == 'supermarket')
                      MidPageOffersWidget(vendorId: restaurant.vendorId ?? restaurant.id ?? 0),

                    // Supermarket-specific sections
                    if (restaurant.businessType == 'supermarket') ...[
                      // Top Products Section
                      TopProductsWidget(vendorId: restaurant.vendorId ?? restaurant.id ?? 0),

                      // Fresh Products (Vegetables & Fruits)
                      FreshProductsWidget(vendorId: restaurant.vendorId ?? restaurant.id ?? 0),

                      // Favorite Snacks
                      FavoriteSnacksWidget(vendorId: restaurant.vendorId ?? restaurant.id ?? 0),

                      // Recently Reviewed Products
                      RecentlyReviewedWidget(vendorId: restaurant.vendorId ?? restaurant.id ?? 0),
                    ],
                  ]),
                ))),

                // Hide "All Items" section for supermarkets - they have their own sections
                if (restaurant.businessType != 'supermarket')
                (restController.categoryList?.isNotEmpty ?? false) ? SliverPersistentHeader(
                  pinned: true,
                                  ...
                                  child: TextField(
                                    ...
                                    decoration: InputDecoration(
                                      ...
                                      prefixIcon: InkWell(
                                        ...
                                        child: Icon(restController.isSearching ? HeroiconsOutline.xMark : HeroiconsOutline.magnifyingGlass,
                          ...
                          restController.type.isNotEmpty ? VegFilterWidget(
                            ...
                            onSelected: (String type) {
                              restController.getRestaurantProductList(
                                restController.restaurant?.vendorId ?? restController.restaurant?.id,
                                1, type, true,
                              );
                            },
                          ) : const SizedBox(),
                ...
                // Hide product list for supermarkets
                if (restaurant.businessType != 'supermarket')
                  ...
                  child: Center(child: Container(
                    ...
                    child: PaginatedListViewWidget(
                      ...
                      onPaginate: (int? offset) {
                        final restaurant = Get.find<RestaurantController>().restaurant;
                        final vendorIdForProducts = restaurant?.vendorId ?? restaurant?.id;
                        if (restController.isSearching) {
                          restController.getRestaurantSearchProductList(
                            restController.searchText, vendorIdForProducts?.toString() ?? '', offset!, restController.type,
                          );
                        } else {
                          restController.getRestaurantProductList(vendorIdForProducts, offset!, restController.type, false);
                        }
                      },
                      ...
                      productView: ProductViewWidget(
                        isRestaurant: false,
                        restaurants: null,
                        products: restController.isSearching ? restController.restaurantSearchProductModel?.products : restController.restaurantProducts,
                        inRestaurantPage: true,
                        businessType: restController.restaurant?.businessType,
                        vendorId: restController.restaurant?.id,
                      ),
      ...
      bottomNavigationBar: GetBuilder<RestaurantController>(builder: (restController) {
          // Don't show bottom cart for pharmacy/supermarket - they have their own in PharmacySupermarketLayoutWidget
          if (restController.restaurant?.businessType == 'pharmacy' || restController.restaurant?.businessType == 'supermarket') {
            return const SizedBox();
          }
          return GetBuilder<CartController>(builder: (cartController) {
            return cartController.cartList.isNotEmpty && !isDesktop ? BottomCartWidget(restaurantId: cartController.cartList[0].product?.restaurantId ?? 0, fromDineIn: widget.fromDineIn) : const SizedBox();
          });
        })
    );
  }
