import 'dart:convert';
import 'dart:io';
import 'package:mnjood/common/widgets/demo_reset_dialog_widget.dart';
...
import 'package:mnjood/features/wallet/controllers/wallet_controller.dart';
...
class NotificationHelper {
  ...
  static Future<void> initialize(FlutterLocalNotificationsPlugin flutterLocalNotificationsPlugin) async {
    ...
    flutterLocalNotificationsPlugin.initialize(initializationsSettings, onDidReceiveNotificationResponse: (NotificationResponse response) async {
      try{
        ...
        if(response.payload!.isNotEmpty) {
          ...
          }else if(payload.notificationType == NotificationType.add_fund || payload.notificationType == NotificationType.referral_earn || payload.notificationType == NotificationType.CashBack || payload.notificationType == NotificationType.wallet_refund){
            Get.toNamed(RouteHelper.getWalletRoute(fromNotification: true));
          }else if(payload.notificationType == NotificationType.product_substitution){
            if(payload.orderId != null) {
              Get.toNamed(RouteHelper.getOrderDetailsRoute(payload.orderId, fromNotification: true));
            }
          }else if(payload.notificationType == NotificationType.order_chat_message){
            if(payload.orderId != null) {
              Get.toNamed(RouteHelper.getOrderChatRoute(payload.orderId!));
            }
          }else{
    ...
    FirebaseMessaging.onMessage.listen((RemoteMessage message) {
      ...
      }else if(message.data['type'] == 'product_substitution'){
        NotificationHelper.showNotification(message, flutterLocalNotificationsPlugin);
        if(Get.find<AuthController>().isLoggedIn() && message.data['order_id'] != null) {
          Get.find<OrderController>().getSubstitutionProposals(int.parse(message.data['order_id'].toString()));
        }
      }else if(message.data['type'] == 'wallet_refund'){
        NotificationHelper.showNotification(message, flutterLocalNotificationsPlugin);
        if(Get.find<AuthController>().isLoggedIn()) {
          Get.find<ProfileController>().getUserInfo();
          Get.find<WalletController>().getWalletTransactionList('1', false, 'all');
        }
      }else if(message.data['type'] == 'order_chat_message'){
        if(Get.find<AuthController>().isLoggedIn() && message.data['order_id'] != null) {
          int orderId = int.parse(message.data['order_id'].toString());
          if(Get.currentRoute == RouteHelper.getOrderChatRoute(orderId)) {
            Get.find<OrderChatController>().refreshMessages(orderId);
          } else {
            NotificationHelper.showNotification(message, flutterLocalNotificationsPlugin);
          }
        } else {
      ...
      try{
        if(message.data.isNotEmpty) {
          ...
          if(notificationBody.notificationType == NotificationType.order) {
            ...
            Get.toNamed(RouteHelper.getSignInRoute(RouteHelper.notification));
          }else if(notificationBody.notificationType == NotificationType.add_fund || notificationBody.notificationType == NotificationType.referral_earn || notificationBody.notificationType == NotificationType.CashBack || notificationBody.notificationType == NotificationType.wallet_refund){
            Get.toNamed(RouteHelper.getWalletRoute(fromNotification: true));
          }else if(notificationBody.notificationType == NotificationType.product_substitution){
            if(message.data['order_id'] != null) {
              Get.toNamed(RouteHelper.getOrderDetailsRoute(int.parse(message.data['order_id']), fromNotification: true));
            }
          }else if(notificationBody.notificationType == NotificationType.order_chat_message){
            if(message.data['order_id'] != null) {
              Get.toNamed(RouteHelper.getOrderChatRoute(int.parse(message.data['order_id'])));
            }
          }else{
            Get.toNamed(RouteHelper.getNotificationRoute(fromNotification: true));
          }
  ...
  static NotificationBodyModel convertNotification(Map<String, dynamic> data){
    ...
    }else if(data['type'] == 'product_substitution'){
      return NotificationBodyModel(notificationType: NotificationType.product_substitution, orderId: data['order_id'] != null ? int.parse(data['order_id'].toString()) : null);
    }else if(data['type'] == 'wallet_refund'){
      return NotificationBodyModel(notificationType: NotificationType.wallet_refund);
    }else if(data['type'] == 'order_chat_message'){
      return NotificationBodyModel(notificationType: NotificationType.order_chat_message, orderId: data['order_id'] != null ? int.parse(data['order_id'].toString()) : null);
    }else {
      return NotificationBodyModel(notificationType: NotificationType.general);
    }
  }
